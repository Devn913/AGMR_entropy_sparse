...
optmParameter.lambda1   = 10^2; %10^2; %10^1;  % YR missing labels 
optmParameter.lambda2   = 10^-7; %10^-7; % W'W embedding
optmParameter.lambda3   = 10^-4; % W Regularization
optmParameter.lambda4   = 10^-5; % R Regularization
optmParameter.lambda5   = 10^-4; % Smoothness in projected input space
optmParameter.lambda6   = 10^-4; % Smoothness in predictions

optmParameter.d1frac    = 0.1; %0.1/0.05 worked great for yeast% fraction of original number of features

optmParameter.etaW      = 0.1; %0.1; %  training rate for W
optmParameter.etaP      = 0.1; %0.1; %  training rate for P
optmParameter.etaR      = 0.1; %0.1; %  training rate for R

optmParameter.maxIter           = 200      ;   % 50-70
...
for dc=1:numel(lambda)
    ...
    cvResult_hyp  = zeros(16,modelparameter.cv_num);
%%
    ...
    for i = 1:modelparameter.repetitions       
        for j = 1:modelparameter.cv_num
           ...
           modelfwmml_hyp  = FWMML_hyp( cv_train_data, IncompleteTarget, optmParameter); 
           %% Prediction and evaluation

            Outputs = (cv_test_data * modelfwmml.W)';
            Outputs_hyp = (cv_test_data * modelfwmml_hyp.W)';
     
            if model_FWMML.tuneThreshold == 1
                fscore                 = (cv_train_data * modelfwmml.W)';
                fscore_hyp                 = (cv_train_data * modelfwmml_hyp.W)';
                [ tau_hyp,  currentResult_hyp] = TuneThreshold( fscore_hyp, cv_train_target', 1, 2);
                [ tau,  currentResult] = TuneThreshold( fscore, cv_train_target', 0, 2);
                Pre_Labels             = Predict(Outputs,tau);
                Pre_Labels_hyp             = Predict(Outputs_hyp,tau_hyp);
                %Predict return 0 and 1's not -1 and 1
            else
                Pre_Labels = sign(Outputs);
                Pre_Labels_hyp = sign(Outputs_hyp);
                %FIX THIS 
                Pre_Labels(Pre_Labels == -1) = 0; %convert -1 to 0, ???
                Pre_Labels_hyp(Pre_Labels_hyp == -1) = 0;
                %Pre_Labels = double(Outputs>=0.5);
            end
            fprintf('-- Evaluation\n');
            tmpResult = EvaluationAll(Pre_Labels,Outputs,cv_test_target');
            tmpResult_hyp = EvaluationAll(Pre_Labels_hyp,Outputs_hyp,cv_test_target');
            cvResult(:,j) = cvResult(:,j) + tmpResult;
            cvResult_hyp(:,j) = cvResult_hyp(:,j) + tmpResult_hyp;
        end
    end

cvResult = cvResult./modelparameter.repetitions;
cvResult_hyp = cvResult_hyp./modelparameter.repetitions;
...
Avg_Result_hyp      = zeros(16,2);
Avg_Result_hyp(:,1) = mean(cvResult_hyp,2);
Avg_Result_hyp(:,2) = std(cvResult_hyp,1,2);
result_hyp =   Avg_Result_hyp;
PrintResults(Avg_Result_hyp);
